...
// ------------------------ End of Open AI functions to create cv data ----------------------------------------------------------------
  ...
  // Function to input the photo and send to the backend
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        const text = await response.text(); // Get error message if available
      ...
      if (data?.filename) {
        handleChange("photo", data.filename);
      }
    } catch (err) {
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    if (response.ok) {
      ...
      a.href = url;
      a.download = `generated_cv.${formData.format}`;
      a.click();
    } else {
