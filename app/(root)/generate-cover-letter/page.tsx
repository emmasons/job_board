...
const searchParams = useSearchParams();
  ...
  useEffect(() => {
    const fetchJobDetails = async () => {
      ...
      try {
        const res = await fetch(`/api/jobs/fetchjob/${jobId}`);
        if (!res.ok) {
  ...
  // ------------------------ Open AI function to create cover data ----------------------------------------------------------------

  const [summaryLoading, setSummaryLoading] = useState(false);

  const handleGenerateCoverLetterBody = async () => {
  const { job_title, company_name, job_applied, full_name, hiring_manager_name } = formData;

    if (!job_title?.trim() || !job_applied?.trim()) {
        alert("Please fill in all the required fields: Job Title and Job Applied.");
        return;
    }
    ...
    try {
        const res = await axios.post("/api/generate-cover-letterbody", {
        job_title,
        company_name,
        job_applied,
        full_name,
        ...
        if (res.data.success) {
        handleChange("cover_letter_body", res.data.content);
        } else {
        alert(res.data.error || "Failed to generate cover letter body");
        }
    } catch (err) {
...
// ------------------------ End of Open AI function to create cover data ----------------------------------------------------------------
