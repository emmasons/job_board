...
export default function CoverDownloadButton({
  cvId,
  fileUrl,
  paymentStatus,
  subscription,
  ...
  const handleDownload = async () => {
    if (!subscription || subscription.status !== "ACTIVE") {
      alert("You need an active subscription to download this Cover letter.");
      return;
    }
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/cover/mark-paid", {
        method: "POST",
      ...
      alert(err.message || "Failed to mark Cover letter as paid.");
    } finally {
      setLoading(false);
    }
  };
